...
export function useSelect(children, onSelect = () => {}, selectOptions) {
  ...
  useEffect(() => {
    if (selected.id === undefined) {
      ...
      defaultSelected.current = {
        ...
        value,
        isDefault: true,
        children: defaultOption.props.children,
      };
      ...
      if (defaultSelected.current.value)
        onSelect(defaultSelected.current, setSelectState);
    } else {
  ...
  function getSelectedOptionChildren(label) {
    ...
    return selected.children && !label ? (
      selected.children
    ) : (
      <SrOnly>Select option</SrOnly>
    );
  }
  ...
  function closeSelect(e) {
    ...
    const selectedOption =
      options.find(({ props: { id } }) => searchedId === id)?.props || selected;
