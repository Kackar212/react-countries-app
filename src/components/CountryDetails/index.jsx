...
export function CountryDetails(props) {
  const { details, name, flag, borderCountries, isLoading } =
    usePrepareCountryData(props);
  ...
  return (
    <>
      ...
      <CountryDetailsWrapper>
        ...
        <BorderCountries>
          <BorderCountriesHeading>Border countries:</BorderCountriesHeading>
          {isLoading && <Loader />}
          {borderCountries &&
            borderCountries.map((name) => (
              <BorderCountry key={name} to={`/${slugify(name)}`}>
                {name}
              </BorderCountry>
            ))}
        </BorderCountries>
